...
reads, we have pre-aligned the \texttt{2cells} data for you using the command below.\\
\begin{warning}
  DO NOT run the following command as you may overwrite the output files of the
  pre-aligned data.
\end{warning}
\begin{lstlisting}
...
\begin{questions}
Using the above command, which we used to pre-align the \texttt{2cells} data, can you
devise and run a command for aligning the \texttt{6h} data?
\\
\\
\\
\\
...
\\
\begin{warning}
  Don’t forget to change the output directory. Otherwise the second command will
  overwrite the results of the previous run.
\end{warning}

Take a look at the output directories that have been created. The results from
Cufflinks are stored in 4 different files named: \texttt{genes.fpkm\_tracking},
\texttt{isoforms.fpkm\_tracking}, \texttt{skipped.gtf} and \texttt{transcripts.gtf}.
\end{questions}
