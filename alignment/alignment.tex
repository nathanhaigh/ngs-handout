...
% TODO Standardise the way we style summeries of software parameters/options
\begin{table}[H]
\centering
\begin{tabular}{>{\raggedleft}p{4cm} p{10.5cm}}
\texttt{--phred33-quals} & Input quals are Phred+33 (default) \\
\texttt{--phred64-quals} & Input quals are Phred+64 (same as \texttt{--solexa1.3-quals}) \\
\texttt{--solexa-quals} & Input quals are from GA Pipeline ver. $<$ 1.3 \\
\texttt{--solexa1.3-quals} & Input quals are from GA Pipeline ver. $\geq$ 1.3 \\
\texttt{--integer-quals} & Qualities are given as space-separated integers (not ASCII)
\end{tabular}
\end{table}
